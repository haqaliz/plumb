---
name: plumb-begin-fast
description: Use when starting work on a Plumb unit of work (a GitHub issue id or an inline brief) and you want the fast path straight to an implementation plan. Triggers on "plumb-begin-fast", "pbf", "pbf bug 12", "pbf feat claim-extraction", "begin fast".
arguments: "type id"
---

# Plumb Begin (Fast Track)

## Overview

Turn a single unit of work into shipped, test-driven code. The fast track is: **isolate → gather → dig → PRD → plan → implement (TDD).** No proposal/diagram deliverables (use `plumb-begin` / `pb` when you need those).
...
**Invocation:** `pbf <type> <id>` — e.g. `pbf bug 12`, `pbf feat claim-extraction`, `pbf chore pin-uv-version`.
...
**REQUIRED SUB-SKILL:** Use `plumb-worktrees`.
...
- Read the saved dump.
- Map the relevant code paths. Plumb is **greenfield** — today only `CLAUDE.md`, `VISION.md`, and `docs/` exist, so for most work the "code paths" are the design docs plus whatever has landed since. Read `docs/technical/CAPABILITY_ROADMAP.md` (the C1..C8 capabilities), `docs/technical/ARCHITECTURE.md` (the design), and `docs/ROADMAP.md` (the phases) to place the work. Once the engine exists it will live under `src/plumb/`; if a `graphify-out/` graph is ever added, query it first per `CLAUDE.md`, then read the files it points to.
...
- Honor the strategic constraints in `CLAUDE.md`. The moat is the **intake → run → re-derive → verdict → bundle** engine plus the compounding discrepancy corpus — **not** a bare LLM judge that opines on correctness. If the work drifts toward a model's opinion standing in for a re-derived value, flag it before planning.
- Place the work in the pipeline: **C1 extract** (deterministic core + optional BYOK proposer), **C2 intake & pinned env**, **C3 execute & capture** (freshness-guarded), **C4 bind & verdict** (the moat), **C5 corpus**, **C6 bundle**, **C7 no-code consistency**, **C8 hosted**. If the work touches verdicts, say so and confirm **execution decides** — a model may extract or propose, never assign `REPRODUCED`.
...
- Confirm a **descriptive** feature slug (kebab-case, e.g. `claim-extraction`) for `docs/planning/{slug}/`. Do **not** name the slug `<type>-<id>` — the id lives in the branch and PR, not in committed doc paths.
...
- Run the suite after each task and keep the branch green: `uv run pytest` for the Python core. If the suite doesn't exist yet, the first task's failing test creates it. Engine tests are deterministic and **run with no network** — any model call sits behind an injectable seam and never runs in CI.
...
- **Phase 1–2:** one agent per related issue/PR (5-line summary + relevance) + one agent to map the affected area (the roadmap/design docs today; `src/plumb/` once it exists). Keep the `gh` calls themselves batched in a single message.
...
| Emitting `DIVERGED` on a harness-side failure | That's `UNVERIFIED` with a named cause; `DIVERGED` requires the artifact's own run to contradict its own claim |
